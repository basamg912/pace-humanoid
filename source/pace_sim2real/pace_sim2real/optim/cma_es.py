...
class CMAESOptimizer:
    def __init__(
        ...
        # 하드웨어 팀에서 제공받은 웨이트에서 크게 벗어나지않도록 regularization 추가 -> evolve 에서 활용
        self.reg_weight = reg_weight
        self.nominal_params = nominal_params  # 하드웨어에서 제공받은 웨이트 [armature, damping, friction, bias, delay]
        ...
        self.scores_buffer = torch.zeros(
            (max_iteration, population_size), device=device
        )
        ...
        if save_optimization_process:
            self.sim_params_buffer = torch.zeros(
                (max_iteration, population_size, bounds.shape[0]), device=device
            )
    ...
    def tell(self, sim_dof_pos, real_dof_pos):
        self.scores += torch.sum(
            torch.square(
                sim_dof_pos - real_dof_pos - self.sim_params[:, self.bias_idx]
            ),
            dim=1,
        )
    ...
    def evolve(self):
        ...
        if self.save_interval > 0 and self.iteration_counter % self.save_interval == 0:
            self.save_checkpoint(
                self._params_to_sim_params(
                    torch.tensor(self.optimizer._mean, device=self.device)
                ),
    ...
    def update_simulator(self, articulation, joint_ids, initial_position):
        ...
        articulation.data.default_joint_armature[:, joint_ids] = self.sim_params[
            :, self.armature_idx
        ]
        ...
        articulation.data.default_joint_viscous_friction_coeff[:, joint_ids] = (
            self.sim_params[:, self.damping_idx]
        )
        ...
        articulation.data.default_joint_friction_coeff[:, joint_ids] = self.sim_params[
            :, self.friction_idx
        ]
        ...
        for drive_type in articulation.actuators.keys():
            ...
            articulation.actuators[drive_type].update_time_lags(
                self.sim_params[:, self.delay_idx].to(torch.int)
            )
    ...
    def _print_iteration(self):
        ...
        print(
            f"Elapsed time: {(datetime.now() - self._timer_start).total_seconds():.1f} seconds"
        )
    ...
    def _log(self):
        ...
        for i in range(len(self.joint_order)):
            ...
            self.writer.add_histogram(
                "3_Friction/distribution_" + self.joint_order[i],
                self.sim_params[:, self.friction_idx][:, i],
                self.iteration_counter,
            )
            self.writer.add_histogram(
                "2_Damping/distribution_" + self.joint_order[i],
            ...
            self.writer.add_scalar(
                "3_Friction/best_" + self.joint_order[i],
                self.sim_params[min_score_index, self.friction_idx][i].item(),
                self.iteration_counter,
            )
            self.writer.add_scalar(
                "2_Damping/best_" + self.joint_order[i],
        ...
        self.writer.add_histogram(
            ...
            self.sim_params[:, self.delay_idx],
            self.iteration_counter,
        )
        self.writer.add_scalar(
            "0_Delay/best",
        ...
        self.writer.add_scalar(
            "0_Episode/score", min_score.item(), self.iteration_counter
        )
        self.writer.add_scalar(
            "0_Episode/max_score", max_score.item(), self.iteration_counter
        )
        self.writer.add_scalar(
    ...
    def save_checkpoint(self, mean, iteration, finished=False):
        ...
        torch.save(
            mean, os.path.join(self.writer.log_dir, "mean_" + f"{iteration:03}" + ".pt")
        )
